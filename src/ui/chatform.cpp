    ...
    ui(new Ui::ChatForm)
{
    ...
    qDebug() << "udp套接字绑定端口："<<get_bind_ip(senderName);
    m_udpSocket->bind(get_bind_ip(senderName),QUdpSocket::ShareAddress | QUdpSocket::ReuseAddressHint);
...
int ChatForm::get_bind_ip(QString userName){
    ...
    QSqlQuery query(db);
    QString sql_str = QString("SELECT * FROM user WHERE nickName='%1'").arg(userName);
    query.exec(sql_str);
    while(query.next()){
